...
from app.schemas.unit_of_measurements import GetAllUnitofMeasurements, CreateUnitofMeasurement, UpdateUnitofMeasurement
from app.services.unit_of_measurements_service import UnitofMeasurementsService
...
async def get_all_apis():
    data = await UnitofMeasurementsService.get_all()
    return await response_base.success(data=data)


@router.get('/{RecId}', summary='Get interface details')
async def get_api(RecId: int):
    api = await UnitofMeasurementsService.get(pk=RecId)
...
async def create_api(obj: CreateUnitofMeasurement):
    await UnitofMeasurementsService.create(obj=obj)
    return await response_base.success()


@router.put('/{RecId}', summary='Update interface')
async def update_api(RecId: int, obj: UpdateUnitofMeasurement):
    count = await UnitofMeasurementsService.update(pk=RecId, obj=obj)
    if count > 0:
        return await response_base.success()
...
async def delete_api(pk: Annotated[list[int], Query(...)]):
    count = await UnitofMeasurementsService.delete(pk=pk)
    if count > 0:
        return await response_base.success()
    return await response_base.fail()
